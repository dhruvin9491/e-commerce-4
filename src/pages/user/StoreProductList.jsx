...
function StoreProductList() {
    ...
    if (error) return <><Header /><main className="catalog"><PageState title="We could not load the store" message={error} /></main><StoreFooter /></>;
                ...
                <div className='catalog'>
                    <div className='section-heading store-toolbar'>
                        <div>
                            ...
                            <h1>Good things, <em>well chosen.</em></h1>
                            <p>{filteredProducts.length} pieces ready to explore.</p>
                        </div>
                        <div className='store-toolbar__controls'>
                            <input type='search' placeholder="Search products" onChange={(e) => setSearch(e.target.value.toLowerCase())} className='store-search' value={search} />
                    ...
                    {!filteredProducts.length && <PageState title="Nothing matches that search" message="Try a different product name or clear the search." />}
                </div>
            </main>
            <StoreFooter />
        </>
